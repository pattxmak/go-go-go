package service

import (
	"bank/dto"
	"bank/errs"
	"bank/logs"
	"bank/repository"
	"strings"
	"time"
)

type accountService struct {
	accRepo repository.AccountRepository
}

func NewAccountService(accRepo repository.AccountRepository) AccountService {
	return accountService{accRepo: accRepo}
}

func (s accountService) NewAccount(customerID int, request dto.NewAccountRequest) (*dto.AccountResponse, error) {

	// validate
	if request.Amount < 500 {
		return nil, errs.NewValidationError("amount at least 500")
	}
	
	if strings.ToLower(request.AccountType)  != "saving" || strings.ToLower(request.AccountType)  != "checking"  {
		return nil, errs.NewValidationError("account type should be saving or checking")
	}

	account := repository.Account{
		CustomerID:  customerID,
		OpeningDate: time.Now().Format("2006-1-2 15:04:05"),
		AccountType: request.AccountType,
		Amount:      request.Amount,
		Status:      1,
	}

	newAcc, err := s.accRepo.Create(account)
	if err != nil {
		logs.Error(err)
		return nil, errs.NewUnexpectedError()
	}

	response := dto.ToAccountResponse(*newAcc)

	return &response, nil

}

func (s accountService) GetAccounts(customerID int) ([]dto.AccountResponse, error) {

	accounts, err := s.accRepo.GetAll(customerID)
	if err != nil {
		logs.Error(err)
		return nil, errs.NewUnexpectedError()
	}

	accResponses := dto.ToAccountResponses(accounts)
	
	return accResponses, nil

}
